...
public class CardSystem : MonoBehaviour
{
    ...
    {
        if (cards.Count == 52) return;                  // 如果撲克牌.數量 等於 52 跳出

        // 跑四個花色
        for (int i = 0; i < type.Length; i++)
        {
            // 跑 1 - 13 張
            for (int j = 1; j < 14; j++)                // 在此段的j = 1，因為要確實讓數字從1 數到 13，不然當腳本丟入場景的空物件時，會有異常的空值
            {
    ...
    public void sort()
    {
        ...
        // select 選取 card
        var sort = from card in cards      // 從cards 這群組裡找所要的東西
